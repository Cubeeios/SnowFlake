...
public class Router: NSObject {
    ...
    public func thpWebView(in routeUrl: String ,delegate:THPWebviewDelegate) -> THPWebview? {
         if routeTable.keys.contains(routeUrl) {
            NSLog("routeUrl---%@", routeUrl);
            let webView = THPWebview();
            //let url = URL(string:routeUrl);
            webView.tpdelegate=delegate;
            _ = webView.webview;
           // webView.initWith(url);
            webView.load(routeUrl);
             return webView;
        }else
         {
